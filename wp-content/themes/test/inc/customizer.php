 ...
 * @return void
 */
function test_customize( $wp_customize ) {
	/**
	 * Initialize sections
	 */
	...
	$wp_customize->add_control(
		new WP_Customize_Image_Control(
			...
			'header_logo',
			array(
				'label'       => __( 'Upload Header Logo', 'test' ),
				'description' => __( 'Height: &gt;80px', 'test' ),
	...
	$wp_customize->add_control(
		...
		array(
			'type'    => 'radio',
			'label'   => __( 'Navbar Scheme', 'test' ),
			'section' => 'theme_header_section',
			'choices' => array(
				'navbar-light bg-light'  => __( 'Default', 'test' ),
				'navbar-dark bg-dark'    => __( 'Dark', 'test' ),
				'navbar-dark bg-primary' => __( 'Primary', 'test' ),
			),
		)
	);
	...
	$wp_customize->add_control(
		...
		array(
			'type'    => 'radio',
			'label'   => __( 'Navbar', 'test' ),
			'section' => 'theme_header_section',
			'choices' => array(
				'static'       => __( 'Static', 'test' ),
				'fixed_top'    => __( 'Fixed to top', 'test' ),
				'fixed_bottom' => __( 'Fixed to bottom', 'test' ),
			),
		)
	);
	...
	$wp_customize->add_control(
		...
		array(
			'type'    => 'checkbox',
			'label'   => __( 'Show Searchfield?', 'test' ),
			'section' => 'theme_header_section',
		)
	);
}
add_action( 'customize_register', 'test_customize' );
